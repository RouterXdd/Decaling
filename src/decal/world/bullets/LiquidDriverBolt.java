...
import decal.world.blocks.distribution.LiquidDriver.*;

import static mindustry.Vars.*;

public class LiquidDriverBolt extends BulletType {

    public LiquidDriverBolt(){
        super(1f, 75);
        collidesTiles = false;
        lifetime = 1f;
    ...
    public void despawned(Bullet b){
        ...
        for(int i = 0; i < data.liquids.length; i++){
            int amountDropped = Mathf.random(0, data.liquids[i]);
            if(amountDropped > 0){
                float angle = b.rotation() + Mathf.range(100f);
                Fx.dropItem.at(b.x, b.y, angle, Color.white, content.liquid(i));
            }
        }
    }
