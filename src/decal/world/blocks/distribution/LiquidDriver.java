...
public class LiquidDriver extends LiquidBlock {
    public float range;
    ...
    public TextureRegion baseRegion = Core.atlas.find(this.name + "-base");

    public LiquidDriver(String name){
        super(name);
        update = true;
        solid = true;
        configurable = true;
        ...
        //point2 is relative
        config(Point2.class, (LiquidDriverBuild tile, Point2 point) -> tile.link = Point2.pack(point.x + tile.tileX(), point.y + tile.tileY()));
        config(Integer.class, (LiquidDriverBuild tile, Integer point) -> tile.link = point);
    }
    ...
    public static class DriverBulletData implements Poolable{
        public LiquidDriverBuild from, to;
    ...
    public class LiquidDriverBuild extends LiquidBuild{
        ...
        public void updateTile(){
            ...
            if(state == DriverState.idle){
                ...
                if(!waitingShooters.isEmpty() && (liquidCapacity - liquids.currentAmount() >= minDistribute)){
            ...
            if(state == DriverState.accepting){
                ...
                if(!hasLink || (!waitingShooters.isEmpty() && (liquidCapacity - liquids.currentAmount() >= minDistribute))){
                    state = DriverState.idle;
                    return;
                }
                ...
                if(
                        liquids.currentAmount() >= minDistribute && //must shoot minimum amount of items
                                link.block.liquidCapacity - link.liquids.currentAmount() >= minDistribute //must have minimum amount of space
                ){
                    LiquidDriverBuild other = (LiquidDriverBuild)link;
        ...
        protected void fire(LiquidDriverBuild target){
            ...
            for(int i = 0; i < content.liquids().size; i++){
                float maxTransfer = Math.min(liquids.get(content.liquid(i)), tile.block().liquidCapacity - totalUsed);
                data.liquids[i] = (int) maxTransfer;
                totalUsed += maxTransfer;
                liquids.remove(content.liquid(i), maxTransfer);
            }
        ...
        protected boolean shooterValid(Building other){
            return other instanceof LiquidDriverBuild entity && other.isValid() && other.efficiency > 0 && entity.block == block && entity.link == pos() && within(other, range);
        }

        protected boolean linkValid(){
            if(link == -1) return false;
            return world.build(this.link) instanceof LiquidDriverBuild other && other.block == block && other.team == team && within(other, range);
        }
